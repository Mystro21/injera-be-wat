...
function Results({ game, onMenu, onRematch }: { game: GameState; onMenu: () => void; onRematch: () => void }) { const scores = calculateFinalScores(game); const winners = game.players.filter((p) => game.winnerIds.includes(p.id)); return <main className={`results-screen ${game.reduceMotion ? 'reduce-motion' : ''}`}><div className="confetti" aria-hidden="true">{Array.from({ length: 28 }, (_, i) => <i key={i} style={{ '--i': i } as React.CSSProperties} />)}</div><span className="eyebrow">THE CIRCLE IS EMPTY</span><h1>{winners.length > 1 ? 'A shared victory!' : `${winners[0]?.name} wins!`}</h1><p>{winners.length > 1 ? winners.map((p) => p.name).join(' & ') : 'The highest point total takes the game.'}</p><div className="score-lesson"><span>FINAL LESSON</span><b>J · Q · K = 10 points each</b><i>Number cards and Jokers = 1 point each</i></div><div className="results-table" aria-label="Final card count and points"><div className="results-row header"><span>Player</span><span>Points</span><span>Cards</span><span>Numbers ×1</span><span>Faces ×10</span><span>Jokers ×1</span></div>{scores.map((score, index) => <div className={`results-row ${index === 0 ? 'winner' : ''}`} key={score.playerId}><span><b>{index + 1}</b>{score.name}</span><strong data-label="Total points">{score.points}</strong><span data-label="Cards collected">{score.cards}</span><span className="score-breakdown" data-label="Number cards">{score.numberCards}<small>= {score.numberPoints} pts</small></span><span className="score-breakdown" data-label="Face cards">{score.faceCards}<small>= {score.facePoints} pts</small></span><span className="score-breakdown" data-label="Jokers">{score.jokers}<small>= {score.jokerPoints} pts</small></span></div>)}</div><div className="result-actions"><button className="secondary-button large" onClick={onMenu}>Return to menu</button><button className="primary-button large" onClick={onRematch}>Play again <span>↻</span></button></div></main>; }
