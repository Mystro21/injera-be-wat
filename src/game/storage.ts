...
export const loadSettings = (fallback: MatchSettings): MatchSettings => { try { const settings = { ...fallback, ...JSON.parse(localStorage.getItem(SETTINGS_KEY) || '{}') }; if (settings.playerName === 'Player') settings.playerName = ''; return settings; } catch { return fallback; } };
